$(document).ready(function () {
            ...
            success: function (response) {
                ...
                if (notificaciones.length > 0) {
                    ...
                    notificaciones.forEach((notif) => {
                        ...
                        if (notif.visto == 0) {
                            nuevas.push(notif);
                        } else if (formatFecha(fechaNotificacion) === formatFecha(hoyFecha)) {
                            hoy.push(notif);
                        } else if (formatFecha(fechaNotificacion) === formatFecha(ayerFecha)) {
                            ayer.push(notif);
                        } else {
                    ...
                    categorias.forEach((categoria) => {
                        if (categoria.notificaciones.length > 0) {
                            $("#notificacion-list").append(`<li class="header">${categoria.nombre}</li>`);
                            categoria.notificaciones.slice(0, 10).forEach((notif) => {
                                ...
                                $("#notificacion-list").append(`
                                    <li style="background-color: white; padding: 10px; border-radius: 5px; margin-bottom: 5px;">
                                        <a href="#" class="notificacion-link" data-seg-id="${notif.id}" style="display: block; text-decoration: none; color: black;">
                    ...
                    if (notificaciones.length > 10) {
                        $("#notificacion-list").append('<li class="footer"><a href="notificaciones.php">Mostrar todas</a></li>');
                    }

                } else {
                    $("#notificacion-list").append(`<li><a href="#">No tienes notificaciones</a></li>`);
                }
            },
    ...
    function marcarNotificacionVisto(seg_id) {
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    window.location.href = `seguimientoID.php?seg_id=${seg_id}`;
                } else {
    ...
    $(document).on("click", ".notificacion-link", function (e) {
        ...
        marcarNotificacionVisto(seg_id);
    });
